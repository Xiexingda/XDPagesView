        ...
        if (newPoint.y >= ADJUSTVALUE) {
            ...
            if (self.pagePullStyle == 0) {
                if (self.mainTable.contentOffset.y <= headerHeight) {
                    ...
                    kvo_scroll.contentOffset = CGPointMake(0, [self topOfScrollView:kvo_scroll]);
                }
            } else if (self.pagePullStyle == 1) {
                if (self.mainTable.contentOffset.y >= ADJUSTVALUE) {
                    _childOffsetStatic = [self topOfScrollView:kvo_scroll]+ADJUSTVALUE;
                    kvo_scroll.contentOffset = CGPointMake(0, [self topOfScrollView:kvo_scroll]+ADJUSTVALUE);
                }
            } else {
                __assert(0,"没有此状态", __LINE__);
            }
        }
    }
}
